...
benchmark/precompile/run.sh /tmp/quantumopticsbase-precompile-results \
    base=/path/to/base/QuantumOpticsBase.jl \
    head=/path/to/head/QuantumOpticsBase.jl
```
...
QuantumOpticsBase consumer Project. The normalized
Manifest must contain exactly one `__QUANTUMOPTICSBASE_CHECKOUT__` path in its
QuantumOpticsBase entry. Setup materializes that placeholder as the stable
...
The total-latency timer starts immediately before `using QuantumOpticsBase` and
...
The copied Manifest uses `__QUANTUMOPTICSBASE_CHECKOUT__` as a path placeholder;
