...
function mul!(result::Operator{B1,B3,T},a::LazyProduct{B1,B2},b::Operator{B2,B3},alpha,beta) where {B1,B2,B3,T}
    ...
    if length(a.operators) == 1
        ...
        tmp1 = Operator(a.operators[end].basis_l,b.basis_r,similar(result.data,dimension(a.operators[end].basis_l),dimension(b.basis_r)))
        mul!(tmp1,a.operators[end],b,a.factor,0)
        for i=length(a.operators)-1:-1:2
            tmp2 = Operator(a.operators[i].basis_l, b.basis_r, similar(result.data,dimension(a.operators[i].basis_l),dimension(b.basis_r)))
...
function mul!(result::Operator{B1,B3,T},a::Operator{B1,B2},b::LazyProduct{B2,B3},alpha,beta) where {B1,B2,B3,T}
    ...
    if length(b.operators) == 1
        ...
        tmp1 = Operator(a.basis_l,b.operators[1].basis_r,similar(result.data,dimension(a.basis_l),dimension(b.operators[1].basis_r)))
        mul!(tmp1,a,b.operators[1],b.factor,0)
        for i=2:length(b.operators)-1
            tmp2 = Operator(a.basis_l,b.operators[i].basis_r,similar(result.data,dimension(a.basis_l),dimension(b.operators[i].basis_r)))
